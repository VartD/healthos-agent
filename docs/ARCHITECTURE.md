...
1. **Telegram Bot (`bot/telegram_bot.py`)**
   ...
   - Каждая команда (например, `/water`, `/glucose`) делает синхронный HTTP POST запрос к Backend API (`/events`), а затем сразу GET запрос (`/analyze`), и возвращает пользователю текстовый ответ.
2. **Backend API (`backend/app/main.py`)**
   - Написан на FastAPI (синхронные маршруты, без async/await).
   - Содержит всего два эндпоинта: `/events` (создание и листинг) и `/analyze` (анализ).
...
4. **База данных**
   ...
   - Единственная таблица: `health_events`.
...
- **Docker Compose**: Есть файл `docker-compose.yml`, который поднимает `postgres` и `backend`. Бот туда не включен (запускается отдельно).
